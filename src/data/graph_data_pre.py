...
def son_smiles_to_graph_dicts(smiles_list, graph_dicts):
    """
    将 SMILES 列表中的子图特征添加到现有的 graph_dicts 中。
    
    :param smiles_list: 要添加的 SMILES 列表
    :param graph_dicts: 现有的特征字典，包括 'atom_info', 'bond_info', 'bond_index'
    :return: 更新后的 graph_dicts
    """
    for smiles in smiles_list:
        if smiles in graph_dicts['atom_info']:
            # 如果子图特征已存在，跳过处理
            continue
        ...
        # 添加新的特征到字典中
        graph_dicts['atom_info'][smiles] = molecule_graph_data.x
        graph_dicts['bond_info'][smiles] = molecule_graph_data.edge_attr
        graph_dicts['bond_index'][smiles] = molecule_graph_data.edge_index

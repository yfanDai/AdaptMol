...
import random
import numpy as np
from src.utils.utils import get_smiles_attribute
...
class MyDataset(data.Dataset):
    ...
    def __init__(self, smiles_tasks_csv, k_spt_pos, k_spt_neg, k_query, tasks, task_num, batchsz,
                 smiles_type='Chemberta', cache_file_prefix="", logger=None, dataname="tox21"):
        ...
        import time
        cache_file_path = f"{cache_file_prefix}/{dataname}.npy"
        ...
        if not os.path.exists(cache_file_path):
            self.all_fps = get_smiles_attribute(list(self.smiles_tasks_csv["remained_smiles"].values))
            self.print("cache smiles attribute to {}".format(cache_file_path))

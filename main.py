...
from src.data.graph_data_pre import smiles_to_graph_dicts, get_graph_data
from src.models.gnn_models import attributes_GNN
from src.models.prototype import get_prototypes, prototypical_loss, get_proto_acc
from src.utils.logger import Logger
from src.utils.utils import plot_attention_heatmap
...
parser.add_argument('--attribute_type', default='smiles', type=str, choices=['smiles'], help='use smiles (ChemBERTa) attributes')
parser.add_argument('--smiles_attributes', default='Chemberta', type=str, help='choose which pretrained model')
parser.add_argument('--attribute_cache_root', default='./cache/', type=str, help='load attributes from attribute_cache_root')
...
parser.add_argument('--emb_dim', type=int, default=300, help='graph embedding dimensions (default: 100)')
...
parser.add_argument('--pretrained_bool', type=str, default='True', help='use pretrained GNN or not')
parser.add_argument('--run_time', default=datetime.datetime.now().strftime("%Y%m%d_%H%M%S"), type=str, help='time')
...
data_train = MyDataset(remained_df, k_spt_pos, k_spt_neg, k_query, tasks[:task_num], task_num, epochs, smiles_type=args.smiles_attributes, cache_file_prefix=args.attribute_cache_root, logger=logger, dataname=args.dataset)
dataset_train = data.DataLoader(dataset=data_train, batch_size=1, shuffle=True)

data_test = MyDataset(remained_df, k_spt_pos, k_spt_neg, k_query, tasks[task_num:], test_task_num, test_batchsz, smiles_type=args.smiles_attributes, cache_file_prefix=args.attribute_cache_root, logger=logger, dataname=args.dataset)
...
for epoch in range(episode):
    ...
    for step, (x_spt, y_spt, a_spt, x_qry, y_qry, a_qry) in enumerate(dataset_train):
        ...
        for i in range(task_num):
            ...
            support_embeddings,_ = model(support_atom, support_bond_index, support_bonds, support_atom_mask_index, a_spt[i])
            ...
            query_embeddings,_ = model(query_atom, query_bond_index, query_bonds, query_atom_mask_index, a_qry[i])
        ...
        if (step+1) % 5 == 0:
            ...
            for x_spt, y_spt, a_spt, x_qry, y_qry ,a_qry in dataset_test:
                ...
                for task_index, (x_spt_one, y_spt_one, a_spt_one, x_qry_one, y_qry_one, a_qry_one) in enumerate(zip(x_spt, y_spt, a_spt, x_qry, y_qry ,a_qry)):
                    ...
                    support_embeddings,support_weights = model(support_atom, support_bond_index, support_bonds, support_atom_mask_index, a_spt_one)  # 这里得到的分子表示不使用属性信息
                    support_embeddings_list.append(support_embeddings)
                    query_atom, query_bonds, query_bond_index, query_atom_mask_index = get_graph_data(x_qry_one, feature_dicts, device=device)
                    query_embeddings,_ = model(query_atom, query_bond_index, query_bonds, query_atom_mask_index, a_qry_one)
                    query_embeddings_list.append(query_embeddings)
                support_embeddings = torch.stack(support_embeddings_list)
                if args.k_shot == 1:
                    prototypes = support_embeddings
            ...
            # plot_attention_heatmap(support_weights)
